...
# 1. Reads YUQUE_API_KEY and NOTION_API_KEY from the process environment.
# 2. Runs both provider sync plans and prints their JSON to stdout.
...
# the matching provider's `acknowledge` command).
set -euo pipefail

cd "$(dirname "$0")/.."

: "${YUQUE_API_KEY:?YUQUE_API_KEY must be exported in the scheduler environment}"
: "${NOTION_API_KEY:?NOTION_API_KEY must be exported in the scheduler environment}"

echo "=== Yuque sync plan ==="
python3 scripts/sync_yuque.py plan

echo "=== Notion sync plan ==="
python3 scripts/sync_notion.py plan
